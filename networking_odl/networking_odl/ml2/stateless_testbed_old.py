...
class NeutronEventHandler():
	...
	def _initialize(self):
		...
		self._ka_count = 0
		self._ka_message_interval = 10000
		self._deque_count = 0
		...
		self._start_deque_timer()
		self._topic = "neutron_events_for_simulation"

	def _start_deque_timer(self):
		self._timer = threading.Timer(self._queue_timeout, self._deque)
	...
	def _deque(self):
		...
		while not self._queue.empty():
			try:
				...
				self._configure_resource(data)
				self._queue.task_done()
			except Exception as ex:
				self._log_handler.error("Encountered Exception:" + str(ex.message))
				continue
		if (self._deque_count % (self._ka_message_interval/10) ==0):
			self._log_handler.debug("Completed dequeue mechanism")
		self._deque_count=self._deque_count+1
		self._start_deque_timer()

	def configure_resource(self,res):
		_event_mapping = {
			"after_create":ttypes.EVENT.CREATE,
			"after_update":ttypes.EVENT.UPDATE,
			"before_delete":ttypes.EVENT.DELETE
		}
		_HANDLERS = {
		...
		try:
			if isinstance(res,dict):
				...
				data = self._convert_to_dict(res.data)
				if (data == None):
					self._log_handler.error("Parsing error of received data")
				else:
					try:
						result = _HANDLERS[res['resource']](_event_mapping[res['event']],**data)
					except KeyError:
						self._log_handler.warning("Event received for an unknown event: " + str(res.type) + ". No processing necessary\n\n")
					except Exception as ex:
						self._log_handler.error("Obtained error when trying to process event: " + str(res.event) + "for resource type: " + str(res.type))
						self._log_handler.error("Resource value: " + str(data))
						self._log_handler.error("Exception: " + str(ex.message))
						return True
			else:
				self._log_handler.error("Unknown resource received. Error!\n\n")
		except Exception as ex:
			self._log_handler.exception("Unhandled exception: " )
		return True
		...
		if (len(params)==2):
			try:
				...
				if (params[1]!="all"):
					...
					param_dict[params[0]][0](ttypes.EVENT.DELETE, val)
				else:
	...
	def handle_switch_creation(self, cmd='start',controller_ip=None, num_switches=10):
		...
		if cmd=='stop' and self._SIMULATION_STARTED:
			...
			message="No virtual computes are started"
		return message
	...
	def __print(self, resourcetype, resource=None ):
		if (resource==None):
			if resourcetype==ttypes.RESOURCE.NETWORK:
				self.__print(resourcetype, self._mininet_data['network'])
			elif resourcetype==ttypes.RESOURCE.SUBNET:
				self.__print(resourcetype,self._mininet_data['subnet'])
			elif resourcetype==ttypes.RESOURCE.PORT:
				self.__print(resourcetype, self._mininet_data['port'])
			elif resourcetype==ttypes.RESOURCE.ROUTER:
				self.__print(resourcetype,self._mininet_data['router'])
			else:
	...
	def _handle_network_event(self, event, **kwargs):
		try:
			network_uuid=None
			result = False
			if event in (ttypes.EVENT.CREATE, ttypes.EVENT.UPDATE):
			...
			elif event==ttypes.EVENT.DELETE:
	...
	def _handle_subnet_event(self, event, **kwargs):
		try:
			result = False
			subnet_uuid = None
			if event in (ttypes.EVENT.UPDATE, ttypes.EVENT.CREATE):
				...
				(result, data)= self._delete_data("subnet", subnet_uuid)
				prefix_length = data["cidr"].split("/")[1]
				...
				return True
			if(result):
				self._log_handler.debug("Subnet event for UUID: " + str(subnet_uuid) + " successful\n\n")
			else:
				self._log_handler.debug("Subnet event for UUID: " + str(subnet_uuid) + " failed\n\n")
			return True
		except Exception as ex:
		...
		try:
			if event==ttypes.EVENT.DELETE:
				...
				if 'fixed_ips' in port_data:
					...
					return True
			elif event in (ttypes.EVENT.UPDATE, ttypes.EVENT.CREATE):
		...
		try:
			if event==ttypes.EVENT.DELETE:
				...
				if(result):
					...
					return True
			elif event in (ttypes.EVENT.CREATE, ttypes.EVENT.UPDATE):
				try:
					port_uuid = kwargs['id']
...
if __name__ == '__main__':
	...
	_RESOURCE_MAPPING = {
		"network": ttypes.RESOURCE.NETWORK,
		"subnet": ttypes.RESOURCE.SUBNET,
		"port": ttypes.RESOURCE.PORT,
		"router": ttypes.RESOURCE.ROUTER
	}
	handler = NeutronEventHandler()
	topic = "neutron_events_for_simulation"
	consumer = KafkaConsumer(bootstrap_servers='localhost:9092', auto_offset_reset='earliest', value_deserializer=lambda m: json.loads(m.decode('utf-8')),group_id="simulator_consumer" )
	consumer.subscribe(topic)
	handler._log_handler.info("Kafka consumer created for topic: " + str(topic))
	try:
		for msg in consumer:
			neutron_resource_event = ttypes.resource()
			...
			try:
				neutron_resource_event.type = _RESOURCE_MAPPING[data['resource']]
				neutron_resource_event.event=_event_mapping[data['event']]
				del data['resource']
				del data['event']
				neutron_resource_event.data = str(data).encode('ascii')
				handler._configure_resource(neutron_resource_event)
	...
	handler._log_handler.info("Thrift server started....")
	while (True):
		try:
			server.serve()
		except KeyboardInterrupt:
			handler._log_handler.info("Session Terminated by the User.")
			transport.close()
			handler._log_handler.info("Closed the Thrift server")
			print("Entering graceful shutdown. Wait... Do not press CTRL-C again. It can corrupt system state")
			time.sleep(2)
			print ("System state is being stored in a DB. Wait... Do not press CTRL-C again. It can lead to corrupted DB")
			handler._log_handler.info("Storing data in the DB before exiting")
			handler.handle_test("subnet", "Stop")
			handler.handle_test("router","Stop")
			result = handler.store_in_db()
